...
-- Ranking DB ?
--====================================================================--
local path = system.pathForFile("ranking2.db", system.DocumentsDirectory)
...
local tablesetup = [[CREATE TABLE IF NOT EXISTS ranking (id INTEGER PRIMARY KEY, jogador, pontos, tempo);]]
db:exec( tablesetup )

function director:gravarPontucaoJogador(player2)
	local jogadorInexistente = true; 
	for jogadoresBase in db:nrows("SELECT * FROM ranking where jogador='"..player2.getPlayerName()..[[']]) do
		 jogadorInexistente = false;
		print('update:'..player2.getPlayerPontos())
local atualizaPontos =[[UPDATE ranking set pontos=']]..player2.getPlayerPontos()..[[', tempo=']]..player2.getPlayerTime()..[[' where jogador=']]..player2.getPlayerName()..[[';]]
		 print(db:exec( atualizaPontos ))
	end
	print(player2.getPlayerName()..player2.getPlayerPontos()..player2.getPlayerTime())
	if (jogadorInexistente  == true) then
         --local jogador = {Nome='Jeferson',Pontos = '10'}                                                            
local insereJogador =[[INSERT INTO ranking VALUES (NULL,']]..player2.getPlayerName()..[[',']]..player2.getPlayerPontos()..[[',']]..player2.getPlayerTime()..[['); ]]
         db:exec( insereJogador )
	end
end

function director:getRanking()
	local jogadores = {}
	count = 0
	for row in db:nrows("SELECT * FROM ranking") do
		count = count + 1
		local playerSetup = PlayerClass.new()
		local jogador1 = playerSetup:setPlayerName(row.jogador)
		jogador1:setPlayerPontos(row.pontos) 
		jogador1:setPlayerTime(row.tempo)
		jogadores[count]=jogador1
	end
	print(#jogadores)
	return jogadores
end

function director:zerarRanking()
	local dropTable = [[ delete from ranking ; ]]
	print(dropTable)
    print(db:exec( dropTable ))
end
...
function director:setLanguage( idioma )
	local languageSetup = require("language")
	local translate = languageSetup.new()
	language=translate:setLanguage(idioma)
end

function director:getLanguage()
	return language
end

function director:setPlayer( playeName)
	player=playeName
--	players[1] = player
end

function director:getPlayer()
	return player
end

players = {}
posicao = 0
function director:addPlayer(nome1,score1,time1)
	local playerSetup = PlayerClass.new()
	local player1 = playerSetup:setPlayerName(nome1)
	player1:setPlayerPontos(score1) 
	player1:setPlayerTime(time1) 
	director:gravarPontucaoJogador(player1)
	jogadorCadastrado = true
	if (posicao > 0) then
		--print('listando Jogadores:')
		for i,j in ipairs(players) do
			if (j.getPlayerName() == player1.getPlayerName()) then
					jogadorCadastrado = false
			end
			--print('jogador'..i..':'..j.getPlayerName())
			--print('score'..':'..j.getPlayerPontos())
			--print('time'..':'..j.getPlayerTime())
		end
	end
	if ( jogadorCadastrado ) then
		posicao = posicao + 1
		players[posicao] = player1
	else
		print('Jogador Ja cadastrado')
	end		
end

function director:getPlayersOnline()
	return players
end
...
function director:getNivel()
	return facil,medium,dificil
end

function director:setNivel(facil1,medium1,dificil1)
	facil = facil1
	medium = medium1
	dificil = dificil1
end
